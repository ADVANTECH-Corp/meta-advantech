...
while getopts "b:k:d:r:R:o:h" o; do
    case "${o}" in
    b)
        BOOTLOADER=${OPTARG}
        ;;
...
if [ -z ${BOOTLOADER}${KERNEL}${DTB}${ROOTFS}${RAMFS} ] ; then
...
if [ ! -z ${RAMFS} ] ; then
    cp ${RAMFS} ${UPDATE_DIR}
    MD5_SUM=`md5sum -b ${RAMFS} | cut -d ' ' -f 1`
    echo "${CMD_RAMFS},${RAMFS},${MD5_SUM}" >> ${UPDATE_DIR}/${UPDATE_SCRIPT}
fi
